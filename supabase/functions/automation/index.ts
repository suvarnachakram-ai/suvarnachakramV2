...
// ✅ Supabase client (requires service role key for writes)
...
const AUTO_DRAW_CONFIG: AutoDrawConfig = {
  enabled: true,
  generateTime: "06:00", // Create daily drafts at 6:00 AM IST
  publishDelayMinutes: 15 // Publish 15 min after draw slot
}

// ✅ Daily draw time slots (IST)
const TIME_SLOTS = ["12:00", "14:00", "17:00"]
...
Deno.serve(async () => {
  try {
    ...
    return new Response(JSON.stringify({ draftMsg, publishMsg }), {
